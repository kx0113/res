...
class WxpayHelper {
  ...
  function __construct($appid,$appkey,$partnerkey,$signtype='sha1') {
    ...
    $log_destination = C('LOG_PATH').'wechat_pay.log';
    $this->logfile = $log_destination;
    Log::write("Diveinto WxpayHelper",'INFO','',$this->logfile);
  }
  ...
  protected function get_cft_package(){
    try {
      ...
      ksort($this->parameters); // 按ASCII码从小到大进行字典排序
  ...
  function get_biz_sign($bizObj){
    Log::write("2222222",'INFO','',$this->logfile);
    foreach ($bizObj as $k => $v){
      $bizParameters[strtolower($k)] = $v;
    }
    try {
      if($this->appKey == ""){
        Log::write("error",'INFO','',$this->logfile);
        throw new SDKRuntimeException("APPKEY为空！" . "<br>");
      }
      Log::write("33333333" . implode('----',$bizParameters),'INFO','',$this->logfile);
      $bizParameters["appkey"] = $this->appKey;
      ksort($bizParameters);
      Log::write("4444444" . implode('----',$bizParameters),'INFO','',$this->logfile);
